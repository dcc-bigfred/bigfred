...
// portsRedisKey is the Redis hash holding allocated port assignments
// so a loco-server restart picks up where the previous run left off.
const portsRedisKey = "dcc-bus:ports"
...
func (d *DccBusService) HydratePorts(ctx context.Context) error {
	...
	rows, err := d.redis.Client().HGetAll(ctx, portsRedisKey).Result()
	if err != nil {
		return fmt.Errorf("hgetall %s: %w", portsRedisKey, err)
	}
	d.mu.Lock()
	defer d.mu.Unlock()
	for k, v := range rows {
		var layoutID, commandStationID uint
		if _, err := fmt.Sscanf(k, "%d:%d", &layoutID, &commandStationID); err != nil {
			continue
		}
...
func (d *DccBusService) Stop(ctx context.Context, layoutID, commandStationID uint) error {
	...
	d.mu.Unlock()
	if d.redis != nil {
		_ = d.redis.Client().HDel(ctx, portsRedisKey, fmt.Sprintf("%d:%d", layoutID, commandStationID)).Err()
	}
	return nil
}
...
func (d *DccBusService) PublishCommand(ctx context.Context, layoutID, commandStationID uint, eventType string, payload any) error {
	...
	channel := fmt.Sprintf("dcc-bus:cmd:%d:%d", layoutID, commandStationID)
...
func (d *DccBusService) persistPort(ctx context.Context, k portKey, port uint16) {
	...
	if err := d.redis.Client().HSet(ctx, portsRedisKey, field, strconv.Itoa(int(port))).Err(); err != nil {
