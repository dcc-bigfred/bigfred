...
func AlloyServiceDef(cfg TelemetryConfig) (ServiceDef, error) {
...
const alloyTemplate = `// Generated by loco-server — BigFred dcc-bus OTLP receiver (gRPC + HTTP).
...
func PrepareAlloyTelemetry(cfg TelemetryConfig) error {
	...
	if err := template.Must(template.New("alloy").Parse(alloyTemplate)).Execute(&content, alloyTemplateData{OTLPEndpoint: endpoint}); err != nil {
