...
func (app *LocoApp) RailroadCp(args RailroadCpArgs) (RailroadCpResult, error) {
	srcDB, err := sql.Open("sqlite", args.SrcFile)
	if err != nil {
		return RailroadCpResult{}, fmt.Errorf("cannot open source database %q: %w", args.SrcFile, err)
	}
	defer srcDB.Close()

	dstDB, err := sql.Open("sqlite", args.DstFile)
	if err != nil {
		return RailroadCpResult{}, fmt.Errorf("cannot open destination database %q: %w", args.DstFile, err)
	}
	defer dstDB.Close()
	...
	if err := row.Scan(ptrs...); err != nil {
		if err == sql.ErrNoRows {
			return RailroadCpResult{}, fmt.Errorf("loco %q not found in %s", args.LocoName, args.SrcFile)
		}
		return RailroadCpResult{}, fmt.Errorf("error reading loco %q: %w", args.LocoName, err)
	}
	...
	if checkErr != nil && checkErr != sql.ErrNoRows {
		return RailroadCpResult{}, fmt.Errorf("error checking destination database: %w", checkErr)
	}

	result := RailroadCpResult{
		LocoName: args.LocoName,
	...
	if checkErr == nil {
		...
		if _, err := dstDB.Exec(updateSQL, updateArgs...); err != nil {
			return RailroadCpResult{}, fmt.Errorf("error updating loco %q in destination: %w", args.LocoName, err)
		}
		result.Updated = true
		...
		if _, err := dstDB.Exec(insertSQL, vals...); err != nil {
			return RailroadCpResult{}, fmt.Errorf("error inserting loco %q into destination: %w", args.LocoName, err)
		}
	}
