...
func TestRosterGateCanDrive(t *testing.T) {
	t.Parallel()
	g := NewRosterGate(2)
	g.ApplySnapshot(layoutroster.AllowedVehicles{
		LayoutID: 2,
		Vehicles: []layoutroster.AllowedVehicle{{
			Addr:              31,
...
func TestRosterGateApplySnapshotIgnoresOtherLayout(t *testing.T) {
	t.Parallel()
	g := NewRosterGate(2)
	if g.ApplySnapshot(layoutroster.AllowedVehicles{LayoutID: 3, Vehicles: []layoutroster.AllowedVehicle{{Addr: 4}}}) {
