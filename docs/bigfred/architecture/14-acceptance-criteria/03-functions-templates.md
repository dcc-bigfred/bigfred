...
- The owner of a vehicle can register **between 0 and 33 functions**
  on it, each with a function number in `F0`–`F32`, a name, an icon
  from the catalogue and a `latched` / `momentary` kind. A request
  with `num=33` or `num=-1` is rejected with `422`.
- The same `num` cannot be registered twice on one vehicle (`409`
  conflict on the unique index).
- The closed list of icons is served by `GET /api/v1/function-icons`
  and matches an asset bundle on the frontend; no other icon string is
  accepted by `PUT .../functions/{num}`.
- A user can create a vehicle template and pick it when registering a
  ...
  template's list with `source: "template"`, and the table
  `vehicle_functions` is empty for that vehicle.
...
- The first edit a user performs on **their vehicle's** functions
  detaches the vehicle: `vehicle_functions` rows are populated from
  the template in the same transaction, `FunctionsDetachedAt` is set,
